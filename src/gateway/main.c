...
/* Выставляется САМИМ потоком перед выходом. Единственный надёжный признак того,
 * что он действительно вышел: pthread_kill(tid, 0) для этого не годится — у
 * незаджойненного потока TID остаётся валидным и после возврата из функции. */
static volatile int g_conn_ticker_done = 0;
 ...
 *
 * Ждём по флагу, который поток выставляет сам, и только потом join — так он
 * пожинается, а не течёт. Раньше здесь стояла проба pthread_kill(tid, 0),
 * которая всегда возвращала 0: цикл каждый раз выкручивал все 6 с и печатал
 * предупреждение даже когда поток завершился мгновенно. Через
 * pthread_timedjoin_np это не делается намеренно — расширение glibc, и на macOS
 * сборка бы отвалилась. */
static void conn_ticker_stop(void) {
    if (!g_conn_ticker_run) return;
    g_conn_ticker_run = 0;
    for (int i = 0; i < 60; i++) {          /* до ~6 секунд */
        if (g_conn_ticker_done) { pthread_join(g_conn_ticker, NULL); return; }
