...
static void kafka_warn_edge_space(const char *name, const char *val)
{
    ...
    /* И в конце. Проверяем ровно на границе: двухбайтовую последовательность с
     * позиции n-2, трёхбайтовую с n-3. Единым циклом было нельзя — при back=3
     * двухбайтовая проверка смотрела бы на n-3..n-2, то есть НЕ на хвост, и
     * давала бы ложное «заканчивается неразрывным пробелом» для строки, где он
     * стоит предпоследним. */
    for (size_t back = 2; back <= 3; back++) {
        if (n < back) continue;
        what = kafka_invisible_at(u + n - back, back);
        /* Совпадение засчитываем, только если оно занимает ровно хвост: для
         * back==3 двухбайтовые шаблоны не годятся. */
        if (what && back == 3 && u[n - 3] == 0xC2) what = NULL;
        if (what) {
