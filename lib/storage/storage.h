...
/* ── Column types ── */
/* Логические типы колонки. Значения ХРАНЯТСЯ по-разному: INT64/BOOL как
 * int64_t, DOUBLE как double, всё остальное — как char* (модель «значение
 * текстом, тип метаданными»).
 *
 * DECIMAL/DATE/TIMESTAMP добавлены В КОНЕЦ намеренно: числовые коды прежних
 * типов не сдвигаются, а каталог хранит типы ИМЕНАМИ, поэтому уже накопленные
 * базы читаются без миграции (неизвестное имя падает в COL_TEXT).
 *
 * Зачем они, если хранение всё равно текстовое: без них десятичное значение
 * приходилось объявлять DOUBLE (двоичная мантисса срезала копейки и длинные
 * идентификаторы) либо TEXT — и тогда на приёмнике создавалась колонка TEXT
 * вместо NUMERIC, а дата — вместо DATE/TIMESTAMP. Тип нужен ровно для того,
 * чтобы приёмник получил родную колонку, а вывод — верное представление. */
typedef enum {
    COL_INT64=0, COL_DOUBLE, COL_TEXT, COL_BOOL, COL_NULL,
    COL_DECIMAL,     /* точное десятичное (numeric/decimal/money), хранится текстом */
    COL_DATE,        /* календарная дата, ISO-8601 YYYY-MM-DD, хранится текстом */
    COL_TIMESTAMP    /* дата со временем, ISO-8601, хранится текстом */
} ColType;

/* Числовой ли тип по смыслу (для сравнений и агрегатов). DECIMAL входит:
 * сравнивать и суммировать его надо как число, а не как строку. */
#define COL_IS_NUMERIC(t) ((t)==COL_INT64 || (t)==COL_DOUBLE || (t)==COL_DECIMAL)
/* Временной ли тип (сравнение лексикографическое — ISO-8601 это допускает). */
#define COL_IS_TEMPORAL(t) ((t)==COL_DATE || (t)==COL_TIMESTAMP)
/* Хранится ли значение как char* (а не как int64_t/double). */
/* Каноническое имя типа и разбор обратно. Раньше отображение было размножено
 * по четырём местам (каталог, две выдачи схемы в API, describe приёмника), и
 * одно из них ИНДЕКСИРОВАЛО массив кодом типа — добавление типа в конец
 * молча читало бы за границей. Держим одну пару на весь проект. */
const char *col_type_name(ColType t);
ColType     col_type_from_name(const char *name);

#define COL_IS_TEXTUAL(t) (!((t)==COL_INT64 || (t)==COL_DOUBLE || (t)==COL_BOOL))
