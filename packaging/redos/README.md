...
чтобы не перебивать уже настроенную). Само имя драйвера задаётся в подключении
полем **«Драйвер ODBC»**: пусто — значит `FreeTDS`; для драйвера Microsoft
пишется `ODBC Driver 18 for SQL Server`.
...
`NTEXT` на 2000 (там `NVARCHAR(MAX)` ещё не существует).
